...
int main() {
    ...
    // ***************************************
    constexpr auto global_num_rows = 8;

    constexpr auto global_num_cols = 8;
    ...
    auto initial_layout = costa::block_cyclic_layout(
            global_num_rows, global_num_cols, initial_block_size, initial_block_size,
            submatrix_start[0], submatrix_start[1], global_num_rows, global_num_cols,
            initial_num_processors_per_row, initial_num_processors_per_col,
            initial_processor_grid_ordering, coordinates_initial_rank[0],
            coordinates_initial_rank[1], &initial_local_data[0], initial_local_values_per_col,
            initial_local_data_ordering, rank);
    ...
    constexpr auto final_num_processors_per_row = 2;
    constexpr auto final_num_processors_per_col = 2;

    constexpr auto final_local_values_per_row = global_num_rows / final_num_processors_per_row;
    constexpr auto final_local_values_per_col = global_num_cols / final_num_processors_per_col;

    auto final_local_data =
            std::vector(final_local_values_per_row * final_local_values_per_col, 0.0);


    constexpr auto final_processor_grid_ordering = 'C';

    constexpr auto final_local_data_ordering = 'R';


    auto final_layout = costa::block_cyclic_layout(
            global_num_rows, global_num_cols, final_block_size, final_block_size,
            submatrix_start[0], submatrix_start[1], global_num_rows, global_num_cols,
            final_num_processors_per_row, final_num_processors_per_col,
            final_processor_grid_ordering, coordinates_initial_rank[0], coordinates_initial_rank[1],
            &final_local_data[0], final_local_values_per_col, final_local_data_ordering, rank);
